...
class CardAggregatePersistenceIT {
    ...
    void save_then_load_round_trips_installments() {
        ...
        CreditCard card = new CreditCard(CardNumber.from("5555555555554444"), new UserId(1L),
                new BankNumber("007"), details, LocalDateTime.now(), LocalDateTime.now(), installments);

        cardRepository.save(card);

        CreditCard reloaded = (CreditCard) cardRepository.findByCardNumber("5555555555554444").orElseThrow();
