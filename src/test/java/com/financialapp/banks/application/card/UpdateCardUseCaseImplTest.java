...
class UpdateCardUseCaseImplTest {
    ...
    private CreditCard buildCreditCard(String cardNumber, LocalDate expiry, int closing, int due) {
        CardDetails details = new CardDetails(
                CardBrand.VISA, CardType.PLATINUM, CardBehavior.CREDIT,
                expiry, new CardBilling(closing, due));
        return new CreditCard(cardNumber, new UserId(1L), BankName.GALICIA,
                details, LocalDateTime.now(), LocalDateTime.now());
    }

    private DebitCard buildDebitCard(String cardNumber, LocalDate expiry, int closing, int due) {
        CardDetails details = new CardDetails(
                CardBrand.MASTERCARD, CardType.STANDARD, CardBehavior.INSTANT_PAYMENT,
                expiry, new CardBilling(closing, due));
        return new DebitCard(cardNumber, new UserId(2L), BankName.SANTANDER,
                details, LocalDateTime.now(), LocalDateTime.now());
    }

    @Test
    void shouldUpdateCreditCardBillingAndExpiry() {
        CreditCard existing = buildCreditCard("1234", LocalDate.of(2026, 1, 1), 15, 10);
        when(cardRepository.findByCardNumberAndUserId(eq("1234"), any())).thenReturn(Optional.of(existing));
        when(cardRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        LocalDate newExpiry = LocalDate.of(2028, 6, 30);
        UpdateCardCommand cmd = new UpdateCardCommand("1234", new UserId(1L), newExpiry, 20, 5);
    ...
    void shouldUpdateDebitCardPartially() {
        DebitCard existing = buildDebitCard("5678", LocalDate.of(2025, 12, 31), 10, 5);
        when(cardRepository.findByCardNumberAndUserId(eq("5678"), any())).thenReturn(Optional.of(existing));
        when(cardRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        // only update closingDay, keep expiry and dueDay
        UpdateCardCommand cmd = new UpdateCardCommand("5678", new UserId(2L), null, 25, null);

        var result = useCase.execute(cmd);

        assertThat(result).isInstanceOf(DebitCard.class);
        assertThat(result.details().expiringDate()).isEqualTo(LocalDate.of(2025, 12, 31));
    ...
    void shouldKeepAllFieldsWhenCommandHasNulls() {
        CreditCard existing = buildCreditCard("9999", LocalDate.of(2027, 3, 15), 12, 8);
        when(cardRepository.findByCardNumberAndUserId(eq("9999"), any())).thenReturn(Optional.of(existing));
        when(cardRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));

        UpdateCardCommand cmd = new UpdateCardCommand("9999", new UserId(1L), null, null, null);

        var result = useCase.execute(cmd);

        assertThat(result.details().expiringDate()).isEqualTo(LocalDate.of(2027, 3, 15));
