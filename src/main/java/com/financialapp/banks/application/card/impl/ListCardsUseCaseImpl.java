...
public class ListCardsUseCaseImpl implements ListCardsUseCase {
    ...
    public List<Card> execute(UserId userId, BankId bankId) {
        if (bankId != null) {
            bankRepository.findByIdForUser(bankId, userId)
                    .orElseThrow(() -> new ResourceNotFoundException("Bank not found: " + bankId.value()));
